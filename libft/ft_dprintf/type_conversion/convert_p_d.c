...
/*   Updated: 2025/12/18 12:28:22 by hanakamu         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
void	ft_putnil(int *counter, t_flag flag_lst)
{
	...
	{
		ft_putspaces(flag_lst.width - 5, counter, flag_lst.fd);
		*counter += write(flag_lst.fd, "(nil)", 5);
	}
	else
	{
...
void	ft_putaddress(unsigned long long ull_num, int fd)
{
	char	*conv_list;

	conv_list = "0123456789abcdef";
	if (ull_num / 16 > 0)
		ft_putaddress(ull_num / 16, fd);
					...
					int *counter, t_flag flag_lst)
{
	if (flag_lst.width <= len)
	{
		...
		ft_putaddress(ull_num, flag_lst.fd);
		*counter += len;
	}
	else if (!flag_lst.is_minus)
	{
