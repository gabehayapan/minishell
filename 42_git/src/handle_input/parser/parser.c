...
/*   Updated: 2026/01/14 16:02:57 by hanakamu         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
void	count_size_exec(t_token *tokens)
{
	t_size_exec	size;
...
int	get_redirect_file(t_token **tokens, t_command *command)
{
	t_rdt	*head_in;
	t_rdt	*head_out;
	t_token	*current;
	...
	while (current != NULL && current->tk_type != AND && current->tk_type != OR
		&& current->tk_type != SEMI && current->tk_type != PIPE)
	{
		if (current->tk_type == SGL_INRDT || current->tk_type == DBL_INPUT)
		{
			if (new_rdt_node(tokens, current, &head_in) == FAILURE)
			{
				free_rdt(head_in);
				return (FAILURE);
			}
		...
		{
			if (new_rdt_node(tokens, current, &head_out) == FAILURE)
			{
				free_rdt(head_in);
				return (FAILURE);
			}
...
char	**new_command(t_token **tokens, t_command *command, t_env *env_lst)
{
	char		**exec;
	int			is_success;

	command->size = count_size_exec(*tokens);
	is_success = get_redirect_file(tokens, command);
	if (is_success == FAILURE)
		return (NULL);
...
t_command	*get_piped_command(t_token **tokens, t_exec *node_exec,
			t_env *env_lst)
{
	t_command	head;
	t_command	*current;
		...
		&& (*tokens)->tk_type != SEMI)
	{
		...
		new_command(tokens, current, env_lst);
		if (head.next == NULL)
		{
			head.next = current;
			last = current;
			...
			last->next = current;
		if (current != NULL && current->tk_type == PIPE)
			clear_token(tokens, current, free);
	}
	return (head.next);
}
...
int	new_exec_tree(t_token **tokens, t_exec **top, t_env *env_lst)
{
	...
	node_exec->command = get_piped_command(tokens, node_exec, env_lst);
